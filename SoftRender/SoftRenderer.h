...
void SoftRenderer::drawPrimitive(const vertex& a, const vertex& b, const vertex& c) {
	...
	MathUtil::perspectiveDivide(p1);//透视除法
	MathUtil::perspectiveDivide(p2);
	MathUtil::perspectiveDivide(p3);

	//转换到屏幕空间
	MathUtil::transformScreen(p1, g_width, g_height);
	MathUtil::transformScreen(p2, g_width, g_height);
	MathUtil::transformScreen(p3, g_width, g_height);
